...
NAME = 'CathayIndex'
ENTRY = 'gui.py'
VERSION_SRC = ['indexer.py']
EXTRA = ['--collect-all', 'tkinterdnd2']
REPO = 'https://github.com/zzhjim02/CathayIndex'
...
- 首个版本：把本地文件夹（含子目录）扫成 SQLite 库「本地文件库」，供 CathayFinder 检索
...
- `CathayIndex.exe` —— 单文件版，约 %.0f MB，双击即用（内含 Python，无需安装）
- `runtime\\`（约 40 MB，自带 Python）+ `建库.bat`（便携版，不想用 exe 时双击它）
